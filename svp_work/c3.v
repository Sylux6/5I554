...
Function sigma_state_map {A B : Type} (f : A -> B) (xs: sigma_state A) : sigma_state B :=
  match xs with
    | OpGet u g => OpGet u (fun x => f (g x))
    | OpSet n g => OpSet n (fun x => f (g x))
  end.
...
Proof.
  ...
  - apply ih_ret.
  - induction s.
    + intros. apply ih_get. intro. apply state_ind_mult.
      * intro. apply ih_ret.
      * intros. apply ih_get. intro. apply H.
      * intros. apply ih_set. intro. apply H.
    + intros. apply ih_set. intro. apply state_ind_mult.
      * intro. apply ih_ret.
      * intros. apply ih_get. intro. apply H.
      * intros. apply ih_set. intro. apply H.
Qed.
...
Proof. intros. unfold bind. reflexivity. Qed.
...
Proof. intros.
       induction mx using state_ind_mult.
       - simpl. reflexivity.
       - intros. simpl. assert (k=(fun n : nat => let! x := k n in ret x)). apply functional_extensionality. intros.
         rewrite H. reflexivity. rewrite <- H0. reflexivity.
       - intros. simpl. assert (k=(fun u : unit => let! x := k u in ret x)). apply functional_extensionality. intros.
         rewrite H. reflexivity. rewrite <- H0. reflexivity.
Qed.
...
Proof.
  ...
  induction mx using state_ind_mult.
  - simpl. reflexivity.
  - simpl. assert ( (fun x : nat => let! y := let! x0 := k x in f x0 in g y)
                    = (fun x : nat => let! x0 := k x in (let! y := f x0 in g y)) ).
    { apply functional_extensionality. intro. apply H. }
    rewrite H0. reflexivity.
  - simpl. assert ( (fun x : unit => let! y := let! x0 := k x in f x0 in g y)
                    = (fun x : unit => let! x0 := k x in (let! y := f x0 in g y)) ).
    { apply functional_extensionality. intro. apply H. }
    rewrite H0. reflexivity.
...
Definition sigma_state_alg {X} (ms: sigma_state (sem_state X)): sem_state X :=
  match ms with
  | OpGet u g => (fun n => g n n)
  | OpSet n g => (fun u => g tt u)
  end
.
...
Fixpoint eval {X}(ms: state X) : sem_state X :=
  match ms with
  | ret v => (fun st => (st, v))
  | op msx => (fun st => sigma_state_alg (sigma_state_map eval msx) st)
  end
.
...
Remark unfold_set {X}: forall (k : unit -> state X) n,
    eval (let! _ := set n in k tt) n = eval (k tt) n.
