...
(** This file is a trimmed down, re-contextualized version of Pierce
    et al. eponymous #<a
    href="https://www.cis.upenn.edu/~bcpierce/sf/current/Hoare.html">chapter</a>#
...
(** You need a working Lecture 2 script *)
Require Import c2.


(** In Lecture 2, we've begun applying the mathematical tools
    ...
    In this lecture, our goal is to carry out some simple examples of
...
Lemma hoare_sub_ex1: {{(fun st => st X <= 5) [X |-> APlus (AId X) (ANum 1)]}}
...
(** However, by using an auxiliary variable [m] to remember the 
    ...
    is correct. Also note that this rule is more complicated than [hoare_asgn].
*)
...
Proof.
  intros m a P.
  unfold hoare_triple. intros. inversion H. destruct H0. subst. rewrite t_update_shadow. rewrite t_update_same. split.
  - assumption.
  - apply t_update_eq.
Qed.
...
Proof.
  ...
  - inversion H. subst. rewrite t_update_shadow. rewrite t_update_same. apply t_update_eq.
Qed.
...
Proof.
  ...
  unfold assn_sub. simpl. rewrite t_update_eq. assumption.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    ...
    + simpl. rewrite t_update_neq. rewrite t_update_eq. reflexivity. unfold not. intros. discriminate. (* can be done with compute *)
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + intros st H. unfold assn_sub. simpl. compute. assumption.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    simpl. intros. destruct H. rewrite Nat.leb_le in H0. omega.
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    simpl. intros. destruct H. rewrite not_true_iff_false in H0. rewrite Nat.leb_gt in H0. omega.
Qed.
...
Check hoare_if.
Theorem hoare_if1 : forall (P : state -> Prop) (Q : Assertion) (b : bexp) (c1 : com),
    {{fun st => P st /\ bassn b st}} c1 {{Q}} ->
    {{fun st => P st /\ ~(bassn b st)}} SKIP {{Q}} ->
    {{P}} (IF1 b THEN c1 FI) {{Q}}.
Proof.
  intros P Q b c1 HTrue HFalse. unfold hoare_triple. intros st st' HE HP. inversion HE.
  - subst. apply (HTrue st).
    + assumption.
    + split.
      * assumption.
      * apply bexp_eval_true. assumption.
  - subst. apply (HFalse st').
    + constructor.
    + split.
      * assumption.
...
Proof.
  ...
  unfold bassn, assert_implies. intros st [Hle Hb].
    simpl in Hb. apply not_true_is_false, leb_iff_conv in Hb. omega.
Qed.
...
Lemma hoare_repeat: (* YOUR STATEMENT HERE *) False. Admitted.
...
(** In this lecture, we have exploited our mechanised syntax &
    semantics of the Imp programming language to define a _program
    logic_ inside Coq. This gave us the ability to write formal
    specifications of imperative programs as well as their
    machine-checked correctness proofs. We have thus used Coq as a
    mathematical language for, first, describing the semantics of a
    programming language and, second, describing a full-blown logical
    system. *)

(** We have thus replaced "paper Mathematics" by "Type Theory", a
    machine-checked formal system for describing any mathematical
    objects we (Computer Scientists/Engineers) may be interested
    in. *)

(** ** Lessons learned *)

(** - Formal systems can (should!) be described in Coq
    - an Hoare triple is nothing but a logical implication
    - Hoare logic is (probably) _not_ a standalone logical system *)

(** ** Take away *)

(** You should be able to:
    - Specify new Imp operators through ad-hoc Hoare triples
    - Prove the correctness of Imp programs *)
