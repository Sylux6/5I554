...
(** **** Exercise: 5 stars (rosetree_induction ')  *)
(**
 ...
 *)
Fixpoint rosetree_ind' (P : rosetree -> Prop)
         (I : forall (n : nat), P (rosenode n nil))
         (IH : forall (n : nat) (t : rosetree) (l : list rosetree),
                      (P (rosenode n l)) -> P t -> (P (rosenode n (cons t l))))
         (t : rosetree) : P t.
Proof.
  intros.
  induction t.
...
Lemma fold_rev_acc (A : Type) : forall (f : A -> A) (l a : list A),
    fold_left (fun (xs : list A) (b : A) => (f b) :: xs) l a = (fold_left (fun (xs : list A) (b : A) => (f b) :: xs) l []) ++ a.
Proof.
  induction l.
  - auto.
  - intro. simpl. rewrite IHl. rewrite IHl with (a := [f a]). rewrite app_assoc_reverse. simpl. reflexivity.
Qed.

Lemma fold_rev_end (A : Type) : forall (f : A -> A) (l a : list A) (x : A),
    fold_left (fun (xs : list A) (b : A) => (f b) :: xs) (l ++ [x]) a =
    (f x) :: (fold_left (fun (xs : list A) (b : A) => (f b) :: xs) l a).
Proof.
  induction l.
  - simpl. reflexivity.
...
Lemma rev_rev_rosetree: 
  forall t, rev_rosetree (rev_rosetree t) = t.
Proof. induction t using rosetree_ind'.
       - simpl. reflexivity.
       - simpl. simpl in IHt. rewrite fold_rev_acc with (a := [rev_rosetree t]). 
         rewrite fold_rev_end. rewrite IHt0. injection IHt. intro. rewrite H. reflexivity.
Qed.
...
*)
Inductive Fml (P : Set) : Type :=
| In  : Fml P
| Not : Fml P -> Fml P
| And : Fml P -> Fml P -> Fml P
| Or  : Fml P -> Fml P -> Fml P
| Imp : Fml P -> Fml P -> Fml P
.
...
Fixpoint phi (t: tree) : tree' :=
  match t with
  | Leaf => Constr (@OpLeaf tree')
  | Node n t1 t2 => Constr (@OpNode tree' n (phi t1) (phi t2))
  end
.

Fixpoint psi (t' : tree') : tree :=
  match t' with
  | Constr (@OpLeaf _) => Leaf
  | Constr (@OpNode _ n t1' t2') => Node n (psi t1') (psi t2')
  end
.
...
Proof.
  intros.
  induction t; try simpl; try auto. rewrite IHt1. rewrite IHt2. reflexivity.
Qed.

(*Unset Implicit Arguments.*)
(** **** Exercise: 5 stars (phi_psi)                                              TODO *)
(** Prove the following lemma: *)
Fixpoint tree'_induction (P : tree' -> Type)
         (I : P (Constr (@OpLeaf tree')))
         (IH : forall (a : nat) (tl tr : tree'),
             P tl -> P tr -> P (Constr (@OpNode tree' a tl tr)))
         (t : tree'): P t :=
  match t with
  | Constr (@OpLeaf _) => I
  | Constr (@OpNode _ a tl tr) => IH a tl tr (@tree'_induction P I IH tl) (@tree'_induction P I IH tr)
  end
.

Lemma phi_psi: forall t, phi (psi t) = t.
Proof.
  induction t using tree'_induction.
  - simpl. reflexivity.
...
Inductive list' : Type :=
| Constl : sigma_list list' -> list'
.

Fixpoint list_list' (l: list A) : list' :=
  match l with
  | nil => Constl (@OpNil list')
  | cons a t => Constl (@OpCons list' a (list_list' t))
  end
.

Fixpoint list'_list (l': list') : list A :=
  match l' with
  | Constl (@OpNil _) => nil
  | Constl (@OpCons _ a tl) => a :: (list'_list tl)
  end
.

Lemma list_to_list' : forall (l: list A), list'_list (list_list' l) = l.
Proof.
  induction l; simpl; auto. rewrite IHl. reflexivity.
Qed.

Fixpoint list'_induction (P : list' -> Type)
         (I : P (Constl (@OpNil list')))
         (IH : forall (a : A) (tl : list'),
             P tl -> P (Constl (@OpCons list' a tl)))
         (l : list'): P l :=
  match l with
  | Constl (@OpNil _) => I
  | Constl (@OpCons _ a tl) => IH a tl (@list'_induction P I IH tl) 
  end
.

Lemma list'_to_list : forall (l' : list'), list_list' (list'_list l') = l'.
Proof.
  induction l' using list'_induction.
  - simpl. reflexivity.
...
Fixpoint fold_list (l : list A) : X :=
  match l with
    | nil => alpha (@OpNil X)
    | a :: tl => alpha (@OpCons X a (fold_list tl))
  end
.
...
Function alpha_length {A : Type} (l : sigma_list A nat) : nat :=
  match l with
  | @OpNil _ _ => 0
  | @OpCons _ _ a t => 1 + t
  end
.
Check fold_list.
Function length {A : Type} (l : list A) : nat :=
  @fold_list A nat alpha_length l
.
...
Function fold_tree (alpha : sigma_tree X -> X) (t : tree) : X :=
  match t with
  | Leaf => alpha OpLeaf
  | Node n t1 t2 => alpha (OpNode n (fold_tree alpha t1) (fold_tree alpha t2))
  end
.
...
Function alpha_height (t : sigma_tree nat) : nat :=
  match t with
  | OpLeaf => 1
  | OpNode _ t1 t2 => 1 + Nat.max t1 t2
  end
.

Function height (t : tree) : nat :=
  fold_tree (alpha_height) t.

Compute height (Node 1 (Node 2 Leaf Leaf) (Node 3 (Node 4 Leaf Leaf) Leaf)).
...
Inductive sigma_ind_tree: tree -> Type :=
  | HLeaf : sigma_ind_tree Leaf
  | HNode : forall (n: nat) (l : tree) (r : tree), P l -> P r -> sigma_ind_tree (Node n l r)
.
