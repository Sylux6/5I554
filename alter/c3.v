...
Definition sigma_state_map {A B} (f : A -> B)(xs: sigma_state A): sigma_state B :=
  match xs with
  | OpGet u g => OpGet u (fun n => f (g n))
  | OpSet n g => OpSet n (fun u => f (g u))
  end.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    assert (k=(fun n : nat => let! x := k n in ret x)).
    apply functional_extensionality.
    intro. rewrite H. reflexivity.
    rewrite <- H0. reflexivity.
  - simpl.
    assert (k=(fun u : unit => let! x := k u in ret x)).
    apply functional_extensionality.
    intro. rewrite H. reflexivity.
    rewrite <- H0. reflexivity.
Qed.
...
Definition sigma_state_alg {X} (ms: sigma_state (sem_state X)): sem_state X :=
  match ms with
  | OpGet _ f => (fun n => f n n)
  | OpSet n f => (fun _ => f tt n)
  end.
