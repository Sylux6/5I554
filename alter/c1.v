...
Fixpoint rosetree_induction (P: rosetree -> Type)
...
Lemma fold_extract_acc: forall (A: Type)(f: A -> A)(l a: list A),
    (fold_left (fun r e => (f e)::r) l a)
    = (fold_left (fun r e => (f e)::r) l []) ++ a.
Proof.
  induction l.
  - simpl. reflexivity.
  - intros. simpl.
    rewrite IHl. rewrite IHl with (a:=[f a]).
    rewrite app_assoc_reverse. simpl.
    reflexivity.
Qed.

Lemma fold_from_end: forall (A: Type)(f: A -> A)(l a: list A)(x: A),
    (fold_left (fun r e => (f e)::r) (l++[x]) a)
      = (f x)::(fold_left (fun r e => (f e)::r) l a).
Proof.
  induction l.
  - simpl. reflexivity.
  - intros. apply IHl.
...
Proof.
  induction t using rosetree_induction.
  - simpl. reflexivity.
  - simpl.
    rewrite fold_extract_acc with (a:=[rev_rosetree t]).
    rewrite fold_from_end.
    rewrite IHt0.
    simpl in IHt. injection IHt. intro. rewrite H.
    reflexivity.
Abort.
...
Fixpoint phi (t: tree): tree' :=
  ...
  | Node n l r => Constr (OpNode tree' n (phi l) (phi r))
  end.

Fixpoint psi (t: tree'): tree :=
  match t with
  | Constr (OpLeaf _) => Leaf
  | Constr (OpNode _ n l r) => Node n (psi l) (psi r)
  end.
...
Fixpoint tree_induction' (P: tree' -> Type)
         (I: P (Constr (OpLeaf tree')))
         (IH: forall (n: nat)(l r: tree'),
             P l -> P r -> P (Constr (OpNode tree' n l r)))
         (t: tree'): P t.
Proof.
  intros.
  induction t.
...
Proof.
  induction t using tree_induction'.
  - simpl. reflexivity.
  - simpl. rewrite IHt1. rewrite IHt2. reflexivity.
Qed.
...
Inductive list' : Type :=
| List': sigma_list list' -> list'.

Fixpoint list_to_list' (l: list A) :=
  match l with
  | nil => List' (OpNil list')
  | (cons e l) => List' (OpCons list' e (list_to_list' l))
  end.

Fixpoint list'_to_list (l: list') :=
  match l with
  | List' (OpNil _) => nil
  | List' (OpCons _ e l) => (cons e (list'_to_list l))
  end.

Lemma list_through_list': forall (l: list A),
...
Fixpoint list_induction' (P: list' -> Type)
         (I: P (List' (OpNil list')))
         (IH: forall (e: A)(l: list'),
             P l -> P (List' (OpCons list' e l)))
         (l: list'): P l :=
  match l with
  | List' (OpNil _) => I
  | List' (OpCons _ e l) => IH e l (list_induction' P I IH l)
  end.

Lemma list'_through_list: forall (l: list'),
    list_to_list' (list'_to_list l) = l.
Proof.
  induction l using list_induction'.
  - simpl. reflexivity.
  - simpl. rewrite IHl. reflexivity.
Qed.
